...
For the full explanation including which datasets are affected, see [DataHawk Data Concepts](/help-center/data-metrics-guides/foundation/key-concepts#amazon-vendor-views-sourcing-vs-manufacturing). The most common related ticket: [I can't see data in Manufacturing views, but I see it in Sourcing](/troubleshooting/data-discrepancies/manufacturing-vs-sourcing-views).
...
Total revenue generated from sales attributed to advertising campaigns. Because it includes sales of products other than the one advertised (see Ad Sales Other SKU below) and is attributed to the ad click date rather than the purchase date, it can be higher than Total Sales for the same product and period. See [why Ad Sales can exceed Total Sales](/troubleshooting/data-discrepancies#ad-spend-or-ad-sales-look-inflated-or-wrong) if you're seeing that.
...
### Sales vs Sales (Orders) vs Gross Revenue vs Sales Principal

These four terms sound interchangeable but aren't — each includes or excludes VAT and shipping differently, and each lives on a different page.

| Term | Where you'll find it | Includes VAT/tax? | Includes shipping? |
|---|---|---|---|
| **Sales** | Ledger category in `FINANCE.FINANCE_PROFIT_LEDGER` (technical reference) | Yes (except US marketplace) | Yes, plus giftwrap |
| **Gross Revenue** | P&L Report tab, Profit Explorer | Yes — including the US, no exception (see note below) | Yes, including shipping |
| **Sales (Orders)** | Seller Analytics dashboard, Sales Report tab (also the basis for ASP and AOV) | Yes (except US marketplace) | No |
| **Sales Principal** | Profit Explorer, grouped by Category → Subcategory, under Sales | No — this is Sales net of VAT | No — shipping and sales tax are broken out into separate subcategories |

Need a net-of-VAT sales figure? Use **Sales Principal** in Profit Explorer — see [Net Sales excluding VAT](/help-center/data-metrics-guides/finance/profitability-analysis) for the full walkthrough. For the full category/subcategory breakdown (including the Marketplace Facilitator VAT lines under Tax Expenses), see [Amazon Seller Profit Data](/help-center/data-reference/amz-seller-profit-data).

<Callout type="warn">
  **Gross Revenue's tax handling is provisional (as of August 2026).** Unlike Sales and Sales (Orders), Gross Revenue doesn't carve out an exception for the US marketplace — it includes VAT/tax everywhere, because the P&L Report and Profit Explorer are still built on an older profit-metrics model that doesn't yet distinguish tax handling by marketplace. A new P&L is planned; Gross Revenue's definition will likely change once that ships and the dashboards are rewired.
</Callout>
...
A pre-built Power BI or Looker Studio dashboard that you customize for your account. DataHawk provides templates for Seller Analytics, Vendor Analytics, Advertising, Market Intelligence, Product, DSP, Search, and Supply Chain. See [Power BI dashboards](/help-center/dashboard-templates/power-bi).
...
DataHawk's modeled estimates of units and revenue for Amazon products, based on rank and price signals. These are estimates, not Amazon-reported sales. They won't exactly match Seller Central. See [Amazon Sales Estimates](/help-center/data-metrics-guides/competition/amz-sales-estimates) for the model details, accuracy expectations, and how to use them.

### LQS — Listing Quality Score

A 0-100 score reflecting how well an Amazon listing follows best practices for titles, images, bullets, and content. Computed daily by DataHawk from 25+ criteria. See [Listing Quality Analysis](/help-center/data-metrics-guides/content/listing-quality-analysis) for the scoring breakdown.
...
DataHawk's REST API configures your **workspace**: adding, removing, or updating tracked products, keywords, and categories. It is not a data-retrieval mechanism, so features and metrics aren't necessarily exposed through it just because they exist in the app. To pull data programmatically instead, use [DataHawk MCP](/help-center/modules/datahawkmcp) or your Snowflake/BigQuery data share. See [API Reference](/api-reference) for what's covered today.

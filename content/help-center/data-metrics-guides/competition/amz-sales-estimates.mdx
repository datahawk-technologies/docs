...
<Callout type="warn">
  **June 15, 2026; V7 model release.** DataHawk now delivers sales estimates at the **variant (child <Term>ASIN</Term>) level**, with rollups to parent and brand. The full 2024+ history was reprocessed with the V7 algorithm on June 15, 2026.
...
Estimates are computed first at the **family (parent ASIN) level**, using the parent sales rank, which is the most reliable signal in the model. The family total is then allocated to each variant using public purchase signals (see [How it works](#how-it-works)). The data is **delivered at the variant (child ASIN) level**, and rolls up to parent and brand for higher-level analysis.
...
<Callout type="info">
  **Partially tracked families.** When you track only one child ASIN from a product family, DataHawk uses the parent ASIN to discover the family structure, but it does not collect sales signals for the untracked children. Estimates for the tracked child rely on that child's own public purchase signals.

  If the tracked child has no public purchase signal, DataHawk treats the absence of signals as a low monthly range of 0-50 units, represented as 25 monthly units. That can happen even when the parent family is a bestseller and the full family sells more than 30,000 units per month.
</Callout>
...
<Step>
### Estimate at the family (parent) level

The model first computes the estimate at the **family (parent ASIN) level**, based on the parent sales rank. This is the most reliable signal available and produces the most accurate value in the model.
</Step>

<Step>
### Distribute to child variants

The family total is then allocated across the children of the family, guided by the strongest evidence available for that family on that day:

- **Direct purchase signal.** When public purchase evidence exists for a variant, its observed share is used.
- **Conservative fallback.** When some children are covered by the catalog but lack their own signal, a conservative fallback share is applied.
- **Equal split.** When no useful child-level signal is available, the family estimate is split evenly across all known variants.

Child estimates are not computed independently: they are allocated from the family total, so the sum of `UNITS_CHILD` across a complete family reconciles with `UNITS_PARENT`. Brand-level views aggregate parent estimates across a brand.
</Step>

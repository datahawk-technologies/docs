...
<Term>BigQuery</Term> is Google's cloud data warehouse — serverless, scalable, and natively integrated with Google's ecosystem (Looker Studio, Google Sheets, Python, etc.). DataHawk uses BigQuery as the hosted database for users who work primarily in Google's tools.

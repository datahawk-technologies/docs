...
<Accordions>
  ...
  <Accordion title="Why are child estimates different from the parent total?">
    Parent estimates represent total family sales. `UNITS_PARENT` is the family-level estimate, computed from the parent sales rank. `UNITS_CHILD` values are allocated from that family total using public purchase signals (with a conservative fallback, or an equal split when no signal is available). When you compare a complete family for the same date, the sum of `UNITS_CHILD` should reconcile with `UNITS_PARENT`. Do not use `UNITS_PARENT` to represent a single variant's sales: it covers the whole family and would count sibling sales onto that ASIN.
  </Accordion>

...
DataHawk organizes datasets into schemas split:

| Schema | Data type | Account required |
|---|---|---|
| `PRODUCT` | Product data (listings, BSR, estimates, rankings) | No |
| `KEYWORD` | Keyword data (search volume, SERP rankings) | No |
| `CATEGORY` | Category structure and rankings | No |
| `SELLER` | Seller Central data (orders, inventory, fees) | Seller account |
| `VENDOR` | Vendor Central data (POs, shipped <Term>COGS</Term>) | Vendor account |
| `ADVERTISING` | Advertising data (campaigns, spend, performance) | Advertising account |
| `REPORTS` | Processed DataHawk report tables | Varies |
...
- Product and keyword data is collected through DataHawk's own proprietary collection infrastructure

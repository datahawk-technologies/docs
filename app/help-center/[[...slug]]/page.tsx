...
export default async function Page(props: { params: Promise<{ slug?: string[] }> }) {
  ...
  const page = helpCenterSource.getPage(params.slug);
  if (!page) notFound();

  const MDX = page.data.body;
  const filePath = join(process.cwd(), 'content/help-center', (page as any).file?.path ?? '');
...
export function generateStaticParams() {
  return helpCenterSource.generateParams();
}
